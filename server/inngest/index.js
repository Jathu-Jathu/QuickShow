...
const sendBookingConfirmationEmail = inngest.createFunction(
    ...
    async ({ event ,step}) => {
        ...
        <p>Enjoy the show! 🍿💥🎬</p>
        <p>Thanks for booking with us!<br/>-- QuickShow Squad</p>
      </div>`,
        });
    }
);
    ...
    async ({ step }) => {
        ...
        const results = await step.run("send-all-reminders", async () => {
            return await Promise.allSettled(
              ...
              <p>
              is scheduled for <strong>${new Date(task.showTime).toLocaleDateString('en-US', { timeZone: 'Asia/Colombo' })}</strong> at
              <strong>${new Date(task.showTime).toLocaleTimeString('en-US', { timeZone: 'Asia/Colombo' })}</strong>
              </p>
              <p>It starts in approximately <strong>8 hours</strong> – make sure you're ready!</p>
              <br/>
              <p>Enjoy the show!<br/>QuickShow Squad</p>
            </div>`,
                    })
                )
            );
...
const sendNewShowNotifications = inngest.createFunction(
    ...
    async ({ event }) => {
        ...
        <br/>
        <p>Thanks,<br/>QuickShow Squad</p>
      </div>`;

            try {
                await sendEmail({ to: user.email, subject, body });
            } catch (err) {
                console.error(`Failed to send new show email to ${user.email}`, err);
            }
        }
